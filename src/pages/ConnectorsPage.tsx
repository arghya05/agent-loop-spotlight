...
import productConnectorsData from '@/data/product/connectors.json';
...
export const ConnectorsPage = () => {
  ...
  if (agentContext === 'product-onboarding') {
    return <ProductConnectorsPage />;
  }
  
  return <SupplierPerformanceConnectorsPage />;
};

// Product Onboarding Connectors
const ProductConnectorsPage = () => {
  const [connectors, setConnectors] = useState(productConnectorsData as any[]);
  const [syncingId, setSyncingId] = useState<string | null>(null);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Plug className="w-5 h-5 text-primary" />
            <h1 className="text-2xl font-bold text-foreground">Product Onboarding Connectors</h1>
          </div>
          <p className="text-sm text-muted-foreground">PIM, DAM, taxonomy, copy AI, compliance vault, and channel publisher integrations</p>
        </div>
        <Button onClick={() => toast.success('Connector added')}><Plug className="w-4 h-4 mr-2" />Add Connector</Button>
      </div>
      <div className="grid grid-cols-2 gap-6">
        {connectors.map((c) => (
          <Card key={c.id} className="card-elevated">
...
// Pricing Intelligence Connectors
const PricingConnectorsPage = () => {
  const [connectors, setConnectors] = useState(pricingConnectorsData as any[]);
  const [syncingId, setSyncingId] = useState<string | null>(null);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Plug className="w-5 h-5 text-primary" />
            <h1 className="text-2xl font-bold text-foreground">Pricing Intelligence Connectors</h1>
          </div>
          <p className="text-sm text-muted-foreground">External competitor, demand, and execution-channel data sources</p>
        </div>
        <Button onClick={() => toast.success('Connector added')}><Plug className="w-4 h-4 mr-2" />Add Connector</Button>
      </div>
      <div className="grid grid-cols-2 gap-6">
...
// Autonomous Inventory Connectors
const InventoryConnectorsPage = () => {
  const [connectors, setConnectors] = useState(inventoryConnectorsData as any[]);
  const [syncingId, setSyncingId] = useState<string | null>(null);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Plug className="w-5 h-5 text-primary" />
            <h1 className="text-2xl font-bold text-foreground">Autonomous Inventory Connectors</h1>
          </div>
          <p className="text-sm text-muted-foreground">ERP, WMS, forecast engines, POS, weather, and supplier EDI integrations</p>
        </div>
        <Button onClick={() => toast.success('Connector added')}><Plug className="w-4 h-4 mr-2" />Add Connector</Button>
      </div>
      <div className="grid grid-cols-2 gap-6">
