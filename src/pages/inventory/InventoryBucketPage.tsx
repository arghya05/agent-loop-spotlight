...
const modeMeta: Record<string, { label: string; color: string; desc: string }> = {
  replenish: { label: 'Replenish', color: 'bg-blue-500/10 text-blue-500', desc: 'OrderAgent triggers reorder via SupplierSystem when stock below safety threshold' },
  transfer: { label: 'Transfer', color: 'bg-purple-500/10 text-purple-500', desc: 'InventoryAgent rebalances network stock via STO before any new purchase' },
  monitor: { label: 'Monitor', color: 'bg-emerald-500/10 text-emerald-500', desc: 'Stock levels sufficient — ForecastAgent continues to monitor demand' },
};

export const InventoryBucketPage = () => {
  const { bucketId } = useParams();
  const navigate = useNavigate();
  const mode = bucketId || 'replenish';
  const meta = modeMeta[mode] || modeMeta.replenish;
  const signals = (signalsData as any[]).filter(s => s.mode === mode);

  return (
    <div className="p-6 space-y-6">
      <Button variant="ghost" size="sm" onClick={() => navigate('/inventory/landing')}>
        <ArrowLeft className="w-4 h-4 mr-1" /> Back to Inventory Home
      </Button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground">On Hand → Forecast</th>
                <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground">Confidence</th>
                <th className="text-left py-2 px-2 text-xs font-medium text-muted-foreground">Savings</th>
            ...
            <tbody>
              {signals.map(s => (
                <tr key={s.id} className="border-b border-border/50 hover:bg-muted/30">
                  ...
                  <td className="py-2.5 px-2">{s.onHand.toLocaleString()} → <span className="font-semibold">{s.forecast14d.toLocaleString()}</span></td>
                  <td className="py-2.5 px-2">{Math.round(s.confidence * 100)}%</td>
                  <td className="py-2.5 px-2 text-status-success font-medium">${(s.expectedSavings / 1000).toFixed(1)}K</td>
